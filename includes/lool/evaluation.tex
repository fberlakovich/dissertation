...
\paragraph{\aflpp only-params}\label{lool:sss:afl-only-params}
The \propername{only-params} variants restrict the input bytes that \aflpp mutates to the generator parameters, removing the program bytes from the mutation space.
...
\begin{table}[!t]
    ...
    \begin{tabular}{>{\nextrow}E C C C C r T T}
        \toprule
        {Experiment} & {Total} & {Passed} & {Timeout} & {Skipped} & {\makecell{Failed \\ (Unique)}} & {\makecell{Med.\ \\ Time (s)}} & {\makecell{Med.\ \\ LoC}} \\
        \midrule
        \startdatarows
