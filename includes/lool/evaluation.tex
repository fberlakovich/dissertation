...
\begin{hypotheses}
    ...
    \end{itemize}
    \hypothesis[hpt:coverage-overhead]{Coverage Feedback Overhead} Context-blind and context-aware coverage incur less overhead than code coverage;
    \hypothesis[hpt:domain-specific-mutation]{Domain-Specific Mutation vs. Generic Mutation} Domain-specific mutation of generation parameters outperforms parametric, but general-purpose mutation with regard to
    \begin{itemize}
        \item triggering rare optimizations;
        \item achieving code coverage.
    \end{itemize}
    \hypothesis[hpt:contextaware-vs-contextblind]{Context-Aware vs. Context-Blind} Context-aware coverage outperforms context-blind coverage with regard to
    \begin{itemize}
        \item triggering rare optimizations;
        \item achieving code coverage.
    \end{itemize}
\end{hypotheses}
In this section we present the results of our experiments and then discuss the hypotheses in \cref{lool:s:discussion}.
...
\begin{table}[t]
    \centering
    \caption{Overview of evaluated configurations.}
    \label{lool:tbl:configurations}
    \small
    \begin{thesistable}{
        colspec = {l l l},
        headrows = 1,
        ...
        \toprule
        Configuration                                               & Mutation Engine   & Coverage Feedback      \\
        \midrule
        \textbf{\ref{config:static})} Static-Params                 & None              & None                   \\
        \textbf{\ref{config:no-feedback})} Random-Params            & Random            & None                   \\
        \textbf{\ref{config:aflcodecov})} \aflpp Code Coverage      & \aflpp            & Code coverage          \\
        \textbf{\ref{config:aflcontextblind})} \aflpp Context-Blind & \aflpp            & Global opt.\ counters  \\
        \textbf{\ref{config:aflcontextaware})} \aflpp Context-Aware & \aflpp            & Per-method opt.\ pairs \\
        \textbf{\ref{config:loolcodecov})} \lool Code Coverage      & Genetic algorithm & Code coverage          \\
        \textbf{\ref{config:loolcontextblind})} \lool Context-Blind & Genetic algorithm & Global opt.\ counters  \\
        \textbf{\ref{config:loolfull})} \lool Context-Aware         & Genetic algorithm & Per-method opt.\ pairs \\
        \bottomrule
    \end{thesistable}
\end{table}
...
We evaluated 8 different configurations enumerated below.
\aflpp configurations, by default, are fully parametric (see \cref{lool:sss:afl-fully-parametric}), whereas \propername{only-params} in the name indicates the variant where the generator reads only generation parameters from the input file (cf. \cref{lool:ss:afl-parameter-mutation}).
...
\begin{configurations}
    ...
    \configuration{\lool Code Coverage}\label{config:loolcodecov} a configuration where we use \lool's domain-specific input mutation (see \cref{lool:s:design}) with code coverage as feedback.
    ...
    \configuration{\lool Context-Aware}\label{config:loolfull} a configuration where we combine per-method optimization pairs as coverage with the domain-specific genetic algorithm for input mutation (see \cref{lool:s:design}).
...
We deliberately chose an \aflpp baseline that uses parametric fuzzing instead of raw mutation of input programs as discussed in \cref{lool:ss:design-overview}.
...
\loadTableData{generated/lool/test-overview}
\begin{table}[!t]
    \footnotesize
    ...
    \resizebox{\textwidth}{!}{%
        \begin{thesistable}[expand=\tabledata]{
            colspec = {l S[table-format=5.0] S[table-format=5.0] S[table-format=5.0] S[table-format=5.0] r S[table-format=3.1,round-precision=1] S[table-format=3.1,round-precision=1]},
            headrows = 1,
            cell{2-Z}{1} = {font=\propernamedecl},
        }
            \toprule
            {Experiment} & {Total} & {Passed} & {Timeout} & {Skipped} & {\makecell{Failed \\ (Unique)}} & {\makecell{Median\ \\ Time (s)}} & {\makecell{Median\ \\ LoC}} \\
            \midrule
            \tabledata
            \bottomrule
        \end{thesistable}}
...
\begin{figure}[p]
    ...
    \begin{subfigure}[t]{\textwidth}
        ...
        \resizebox{0.85\textwidth}{!}{%
            ...
            \pgfplotsset{every axis/.append style={
                ...
                legend style={overlay, draw=none, fill=none, text opacity=0},
            }}
    ...
    \caption{Cumulative edge coverage over time.}
    \label{lool:fig:cumulative-coverage-combined}
\end{figure}
...
\begin{itemize}
    ...
    \item number of rare optimizations performed (see \cref{lool:ss:eval-rare-opts})
    \item number of rare optimization pairs performed (see \cref{lool:ss:eval-rare-opts})
\end{itemize}
...
We discuss this phenomenon in more detail in \cref{lool:ss:aflpp-vs-genetic}.

\subsection{Overhead of Feedback Mechanisms}

\begin{figure}[t]
    \centering
    \resizebox{\textwidth}{!}{\input{figures/lool/coverage-overhead}}
    \caption{Relative throughput of different coverage feedback mechanisms in a 30-minute measurement period.
    Absolute test counts are shown in parentheses; \propername{none} (169 tests) is the baseline.}
    \label{lool:fig:coverage-overhead}
\end{figure}
\begin{figure}[t]
    \centering
    \resizebox{\textwidth}{!}{%
        \begingroup
        \pgfplotsset{every axis/.append style={
            yticklabel style={font=\propernamedecl},
    ...
    \label{lool:fig:rare-pairs}
    See \cref{lool:ss:eval-rare-opts} for details.}
\end{figure}
...
\cref{lool:fig:coverage-overhead} shows the relative throughput for each coverage type.

\subsection{Code Coverage}\label{lool:sss:eval-code-coverage}
%\begin{table}[!t]
%
%    \caption{Cumulative coverage development over time for the better performing half of configurations (see \cref{lool:fig:cumulative-coverage-upper}).}
...
\subsection{Triggering Rare Optimizations}\label{lool:ss:eval-rare-opts}
...
The boxplots in \cref{lool:fig:rare-opts,lool:fig:rare-pairs} illustrate statistics based on the rareness scores of the runs in a configuration.
The boxplot in \cref{lool:fig:rare-opts} is based on optimization rareness scores and the boxplot in \cref{lool:fig:rare-pairs} is based on optimization-pair rareness scores.
