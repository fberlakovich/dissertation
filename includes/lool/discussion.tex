...
In the variants that mutate only generation parameters (see \cref{lool:sss:afl-hyperonly}), the generator reads a short, fixed byte sequence from the \aflpp generated input, making it easier for \aflpp to detect which bits have an effect.
...
As expected, this effect is mitigated in the variant that reads only generation parameters from the seed (see \cref{lool:sss:afl-hyperonly}).
