...
In the variants that mutate only generation parameters (see \cref{sss:afl-only-params}), the generator reads a short, fixed byte sequence from the \aflpp generated input, making it easier for \aflpp to detect which bits have an effect.
...
As expected, this effect is mitigated in the variant that reads only generation parameters from the seed (see \cref{sss:afl-only-params}).
...
Cumulative code coverage (\cref{lool:fig:cumulative-coverage}) and rareness scores (\cref{fig:rare-opts,fig:rare-pairs}) suggest that context-aware coverage is less beneficial than context-blind coverage.
