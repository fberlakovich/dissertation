\section{Discussion}\label{lool:s:discussion}

\input{generated/lool/defines}

\subsection{Coverage vs. No Coverage}
The main advantage of having no coverage at all is the higher execution speed, as evidenced by the data in \cref{lool:fig:coverage-overhead}.
On the other hand, coverage feedback enables guidance through feedback-directed input mutation.
\cref{lool:fig:cumulative-coverage} shows that \propername{random-params}, \ie without feedback but with random generation parameters, achieves the highest cumulative code coverage of all configurations.
This advantage in code coverage is not, however, reflected in the achieved rare optimization scores (see \cref{lool:fig:rare-opts,lool:fig:rare-pairs}).
...
%The strong code coverage performance of \propername{random-params} can be explained by a "depth vs.\ breadth" trade-off.
%The \propername{random-params} configuration, being unburdened by feedback overhead, achieves a high throughput and rapidly explores the "breadth" of the compiler's state space---triggering standard compilation phases (parsing, high-level IR generation) across many diverse inputs.
%In contrast, the feedback-driven genetic algorithm optimizes for "depth"---specifically aiming for rare optimizations and complex phase interactions that occur less frequently.
%While the random approach covers more code lines in total (\eg error handling or common paths), the genetic algorithm successfully pressures the compiler into deep optimization pipelines that a random search rarely reaches, as evidenced by the higher rareness scores.
...
We attribute the improvement of the unguided configurations over \aflpp to the more structural issues with \aflpp, which we discuss in \cref{lool:ss:aflpp-vs-genetic}.
...
\subsection{\aflpp vs. Genetic Parameter Mutation}\label{lool:ss:aflpp-vs-genetic}

\loadTableData{generated/lool/afl-test-overview}
\begin{table}[!t]
    \centering
    \begin{thesistable}[expand=\tabledata]{
        colspec = {l S[table-format=5.0] S[table-format=5.0] S[table-format=5.0]},
        headrows = 1,
        cell{2-Z}{1} = {font=\propernamedecl},
    }
        \toprule
        {Experiment} & {Total} & {Skipped} & {Identical} \\
        \midrule
        \tabledata
        \bottomrule
    \end{thesistable}
    \caption{The number of total, skipped, and identical test runs for \aflpp configurations.}
    \label{lool:tbl:afl-overview}
\end{table}

The cumulative coverage in \cref{lool:fig:cumulative-coverage} indicates that our custom genetic algorithm outperforms the \aflpp and \propername{static-params} configurations in terms of code coverage.
...
\cref{lool:fig:rare-opts,lool:fig:rare-pairs} show that \propername{genetic-code} nonetheless triggers a similar amount of rare optimizations and pairs.
...
The number of identical tests and re-generation attempts are shown in \cref{lool:tbl:afl-overview} and \cref{lool:tbl:afl-attempts}, respectively.
...
In the variants that mutate only generation parameters (see \cref{lool:sss:afl-only-params}), the generator reads a short, fixed byte sequence from the \aflpp generated input, making it easier for \aflpp to detect which bits have an effect.
...
\cref{lool:tbl:afl-overview} shows the number of identical tests summed over all runs of an experiment for each of the \aflpp configurations.



\loadTableData{generated/lool/afl-test-attempts}
\begin{table}[!t]
    \centering
    \begin{thesistable}[expand=\tabledata]{
        colspec = {l S[table-format=5.0] S[table-format=5.0] S[table-format=5.0] S[table-format=5.0] S[table-format=5.0] S[table-format=5.0]},
        headrows = 1,
        cell{2-Z}{1} = {font=\propernamedecl},
    }
        \toprule
        {Experiment} & {First} & {Once} & {Twice} & {Thrice} & {Fourth} & {Fifth} \\
        \midrule
        \tabledata
        \bottomrule
    \end{thesistable}
    \caption{How many tests were successfully generated on the first try or had to be retried with reduced generation parameters.}
    \label{lool:tbl:afl-attempts}
\end{table}
...
In cases where there is too little randomness for the generator's random decisions, the generator tries to size down the generated test case by adapting structural parameters (see \cref{lool:sss:afl-fully-parametric}).
We found that in the fully parametric variant (see \cref{lool:sss:afl-fully-parametric}) the generator reduced the parameters of almost all tests at least once.
\cref{lool:tbl:afl-overview} shows the number of tests with reduced generation parameters for each configuration.
As expected, this effect is mitigated in the variant that reads only generation parameters from the seed (see \cref{lool:sss:afl-only-params}).
...
The \enquote{Skipped} column in \cref{lool:tbl:test-overview} shows the number of tests that had to be skipped due to such an unusable seed file.
...
This difference is reflected in the low calibration times of 2\% and 4\% of the total runtime in \propername{afl-contextblind} and \propername{afl-contextblind-hyperonly}.
\propername{afl-contextblind} loses this advantage again, however, by spending 90\% of the total runtime in trimming.

Configurations using the genetic algorithm consistently trigger more rare optimizations and optimization pairs than their \aflpp counterparts (see \cref{lool:fig:rare-opts,lool:fig:rare-pairs}).
...
\cref{lool:fig:coverage-overhead} shows that coverage feedback based on the optimization log (either global counters or per-method counters) has a significantly lower overhead than code coverage.
...
\end{infobox}
At the same time, \cref{lool:fig:cumulative-coverage} shows that configurations based on code coverage consistently achieve less overall code coverage than configurations based on the optimization log, regardless of the mutation strategy.
...
\cref{lool:fig:rare-opts,lool:fig:rare-pairs} show that code coverage feedback triggers fewer rare optimizations and optimization pairs.
...
\subsection{Context-Aware Coverage vs. Context-Blind Coverage}\label{lool:ss:context-aware-vs-context-blind}
\loadTableData{generated/lool/optimization-scores}
\begin{table}[!t]
    \centering
    ...
    \footnotesize
    \begin{thesistable}[expand=\tabledata]{
        colspec = {l S[table-format=6.1,round-precision=1] S[table-format=6.1,round-precision=1] S[table-format=5.0]},
        headrows = 1,
        cell{2-Z}{1} = {font=\propernamedecl},
    }
        \toprule
...
Cumulative code coverage (\cref{lool:fig:cumulative-coverage}) and rareness scores (\cref{lool:fig:rare-opts,lool:fig:rare-pairs}) suggest that context-aware coverage is less beneficial than context-blind coverage.
...
In addition to the configurations listed in \cref{lool:s:evaluation}, we also evaluated variants of the different configurations.
