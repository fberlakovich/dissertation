...
Despite unsolved problems, the security community's interest in code-reuse mitigations seems to decline, judging by the publication counts,
...
While hardware defenses like Intel CET and ARM PAC have reduced the urgency for software-only solutions, they are not a silver bullet.
...
The following two subsections describe possible future research directions.
...
If, however, an attacker targets only one particular instance, they might be willing to accept higher reconnaissance costs and even manual target reconnaissance.
...
Given these mutually exclusive strengths and weaknesses, it is surprising that there is little research on the composability of different defense strategies.
...
Similarly, the Readactor authors speculated about the possibility of a whole-function-reuse attack, but without \gls{AOCR}'s malicious thread blocking and pointer profiling, such an attack seemed infeasible at the time.
...
The \propername{Survivor} algorithm provided a comparable metric for early variants of code randomization, focusing on reducing the risk of \gls{ROP} gadgets.
...
We gave our best to underpin the claims in \cref{ch:r2c} with empirical data, but a more foundational theory of software diversity remains desirable.
Thus, we hope that future researchers take up the challenge of formulating such a theory of software diversity.
