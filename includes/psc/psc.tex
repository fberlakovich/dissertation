...
We have also introduced one of the blind spots of compiler-based code coverage instrumentation---indirect calls.
A closer look reveals that this particular blind spot is an instance of a more general issue.
...
\glspl{JITcomp} perform a similar optimization called \emph{polymorphic inlining} (see \cref{psc:subsec:polymorphic-inlining}), typically based on profiling information.
...
Control-flow in programs typically depends on the value of, \eg variables.
An \code{if} might check whether a variable \code{x} is above a certain bound or a loop starts at a particular value and iterates until the index variable reaches a limit.
Conventional edge coverage already covers these types of control-flow decisions.
From the perspective of edge-coverage, each control-flow point partitions the value range of the compared variable depending on the comparison operator.
...
\end{listing}
The function \code{target} has different logic depeonding on whether \code{param} is positive or negative.
A \code{uint16\_t}, however, has a value in the range $[-32768, 32767]$, meaning that from the perspective of edge coverage all values in the ranges $[-32767, -1]$ and $[0, 32767]$ are treated uniformly.
Whichever value in one of those ranges the fuzzer explores, it will not trigger new coverage in \code{target}.
This blindspot can lead to cases where a fuzzer might struggle to find certain interesting data points because there is no direct control dependency on them.
...
For example, compilers use such analyses to eliminate redundant conditions like in \cref{psc:line:redundant-condition} in \cref{psc:lst:redundant-condition}.
\begin{listing}
...
\end{listing}
Assuming that \code{param} is not, \eg \code{volatile}, the compiler can prove that at the time control-flow reaches the condition at \cref{psc:line:redundant-condition}, \code{param} can never be $0$.

We can use this compiler knowledge to map these hidden dataspace boundaries onto the control-flow, again making them available for edge-coverage instrumentation.
For example, a simple compiler transformation can transform \cref{psc:lst:c-value-range} into \cref{psc:lst:c-value-range-transformed}.
From the perspective of edge coverage, the transformation effectively partitions the data space into four partitions instead of two.
...
\begin{description}
    \item[midpoint-splitting] The heuristic described above simply splits value ranges in the middle, creating two roughly equally sized partitions.
    This heuristic makes sense if nothing is known about the value range in question.
    \item[edge-splitting] This heuristic splits slivers off the ends of the value range.
    The reasoning behind focusing on the value range boundaries is that bugs often hide behind extreme values (\eg off-by-one errors).
    ...
    Apart from values at the range's border, which values in a range are interesting also depends on the type of operation a value occurs in.
    For example, certain values in an \code{add} instruction might cause an overflow, which in turn the program might not correctly handle.
    Another example are special floating point values such as \code{NaN} and \code{Inf}.
    ...
    Lookahead-splitting transforms the program into the program shown in \cref{psc:lst:lookahead-splitting-example-transformed}.
\end{description}
