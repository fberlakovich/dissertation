...
The history of software security is characterized by a continuous cycle of new attack paradigms and corresponding defensive responses.
This coevolutionary process has led to a highly sophisticated landscape of memory corruption exploits and mitigations.

Code-injection attacks and \glspl{CRA} typically proceed in three phases:
\begin{inlinenum}
    \item a reconnaissance phase where the attacker collects information about the target, such as addresses,
...
Defenses against such attacks can be categorized into three categories:
\begin{inline}
...
The following sections give a historical overview of the development of \glspl{CRA} and how defenses in each category evolved with them.

\section{Code Injection (1997--2004)}
\label{sec:epoch-injection}
\begin{figure}[t]
    \centering
    \input{generated/r2c/coevolution-epoch-code-injection.tex}
    \caption{Epoch timeline for code-injection attacks and defenses (1997--2004).}
    \label{fig:coevolution-epoch-code-injection}
\end{figure}
In the earliest era, the adversary's primary goal was to inject new code (\eg shellcode) into the target process' memory space and subsequently redirect execution to the injected code.
...
Another issue with \gls{ASLR} is that a 32-bit address space does not offer enough entropy, and even in a 64-bit address space, pointer leaks and side-channels make \gls{ASLR} vulnerable.
Research into \gls{ISR} explored randomizing the processor's instruction set to prevent the execution of injected code~\cite{boyd2010}.
...
\begin{figure}[t]
    \centering
    \input{generated/r2c/coevolution-epoch-static-rop.tex}
...
\end{figure}
With \gls{DEP} preventing code injection, attackers were forced to reuse code already present in the process memory.
...
\gls{ROP} was later generalized to other architectures~\cite{roemer2012,cloosters2022,jaloyan2020} and to gadgets with other free-branch instructions~\cite{checkoway2010,Bletsch2011a}, as well as returnless variants that avoid the return stack entirely~\cite{checkoway2010}.
...
Implementations like \propername{CFI for COTS Binaries} demonstrated that such policies could be applied to stripped binaries using static binary rewriting, albeit with the aforementioned precision limitations~\cite{zhang2013}.
...
To reduce the resulting attack surface, \propername{Control-Flow Locking} refines the \gls{CFI} policy by dynamically locking and unlocking allowed targets~\cite{Bletsch2011b}.
...
Parallel to \gls{CFI}, memory-safety techniques try to solve the problem at the root of many attack classes, including \glspl{CRA}: memory corruption.
...
As a response to the new threat of \gls{ROP} attacks, researchers proposed finer-grained (compared to \gls{ASLR}) randomization techniques.
...
To make randomization more practical for deployment, \propername{Compiler-Assisted Code Randomization} proposed embedding metadata to allow fine-grained diversity without heavy runtime overhead~\cite{Koo2018}.
...
\begin{table}[ht]
    ...
    \begin{tabular}{@{}llp{4cm}p{4cm}@{}}
        ...
        Function Perm. & Function & Function Reuse (\eg \propername{ret2libc}) & Vulnerable to \gls{ROP}; Partial overwrites \\
...
\begin{figure}[t]
    \centering
    \input{generated/r2c/coevolution-epoch-dynamic-semantic.tex}
...
\end{figure}
The third epoch was driven by the realization that \emph{static} defenses cannot withstand \emph{dynamic} attacks.
Another group of attacks exploited the fact that several \gls{CFI} defenses did not sufficiently restrict an attacker's mobility or had a narrow semantic scope, \eg not considering \cpp semantics.
...
Early systems like \propername{XnR}~\cite{Backes2013} and \propername{HideM}~\cite{Goktas2016b} pioneered this approach by emulating execute-only pages in software or using split-TLB architectures.
...
For example, \propername{Readactor} combines fine-grained randomization with execute-only memory and \emph{booby traps}~\cite{Crane2015}.
...
\propername{Shuffler}, for example, re-randomizes code layout at short intervals (50\,ms), introducing a tight deadline for disclosure~\cite{WilliamsKing2016}.
Similarly, \propername{TASR} rerandomizes at regular intervals, but instead of a translation table updates pointers inplace~\cite{bigelow2015}.
...
Just as static randomization failed against dynamic inspection, early implementations of \gls{CFI} were challenged by the inherent imprecision of static analysis.
...
This overapproximation gives attackers sufficient maneuverability to construct malicious execution paths that adhere to the statically allowed edges, as demonstrated by a series of attacks on \gls{CFI}~\cite{Carlini2015,Evans2015a}.
...
\gls{COOP} uses virtual function calls to perform malicious computation, but from the perspective of a \cpp-oblivious \gls{CFI} \gls{COOP} stays within the legitimate \gls{CFG}.
\begin{listing}[ht]
\begin{minted}{C++}
...
\end{listing}
In response, researchers have proposed \gls{CFI} solutions specialized for object-oriented languages~\cite{Gawlik2014,Zhang2015,Prakash2015,wang2018a}.
For example, \propername{CFIXX} enforces strict object-type integrity on virtual calls to prevent COOP-style attacks~\cite{Burow2018CFIXX}.
...
Researchers also attempted to generalize the separation of control-flow data and to generally separate \enquote{safe} from \enquote{unsafe} data.
...
This inlining also means, however, that the security provided by \gls{PAC} is limited by the available unused bits in a memory address.
...
This chapter has traced decades of coevolution between code-reuse attacks and defenses.
...
\gls{CFI} accommodates the presence of corruption but is constrained by the inherent imprecision of static analysis and subsequent gaps in semantic coverage.
...
The following chapter investigates one such vulnerability: the predictability of observable data within randomized environments.