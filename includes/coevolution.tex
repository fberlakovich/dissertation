...
\begin{inline}
    \item a reconnaissance phase where the attacker collects information, such as addresses about the target,
...
\begin{figure}[p]
    ...
    \resizebox{!}{0.88\textheight}{%
        \input{generated/coevolution/coevolution-epoch-all}%
    }
...
\begin{figure}[H]
    \centering
    \resizebox{\textwidth}{!}{%
        \input{generated/coevolution/coevolution-epoch-code-injection}%
    }
...
\begin{figure}[H]
    \centering
    \resizebox{\textwidth}{!}{%
        \input{generated/coevolution/coevolution-epoch-static-rop}%
    }
...
\gls{ROP} was later generalized to other architectures~\cite{roemer2012,cloosters2022,jaloyan2020} and to alternative dispatch mechanisms~\cite{checkoway2010,Bletsch2011a,bosman2014}, as well as returnless variants that avoid the return stack entirely~\cite{checkoway2010}.
...
Implementations like \enquote{CFI for COTS Binaries} demonstrated that such policies could be applied to stripped binaries using static binary rewriting, albeit with the aforementioned precision limitations~\cite{zhang2013}.
...
To make randomization more practical for deployment, \enquote{Compiler-Assisted Code Randomization} proposed embedding metadata to allow fine-grained diversity without heavy runtime overhead~\cite{Koo2018}.
...
\begin{figure}[H]
    \centering
    \resizebox{\textwidth}{!}{%
        \input{generated/coevolution/coevolution-epoch-dynamic-semantic}%
    }
...
This overapproximation gives attackers sufficient maneuverability to construct malicious execution paths that adhere to the statically allowed edges, as demonstrated by a series of attacks on \gls{CFI}~\cite{Carlini2015,Evans2015a,Goktas2016b}.
...
Similarly, \enquote{\citetitle{biondo2018}} shows that implementation tradeoffs for performance can have unexpected consequences~\cite{biondo2018}.
...
\propername{CFI-LB} augments the statically determined \gls{CFG} with runtime context at the call site, such as a chain of return addresses on the stack.
