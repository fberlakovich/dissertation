...
By inserting booby-trapped return addresses that are indistinguishable from real ones, \rtwoc forces attackers to guess among decoys, dramatically increasing the difficulty of reconnaissance-based attacks.
Our evaluation on large-scale applications including Chromium and WebKit demonstrated that this protection comes at a moderate performance cost, making \rtwoc practical for real-world deployment.

\paragraph{LOOL: Optimization-Aware Compiler Fuzzing}
In Part~\ref{part:fuzzing}, we presented \lool, a fuzzing framework that uses the compiler's optimization log as a domain-specific coverage signal.
...
A genetic algorithm uses this feedback to breed input generators that trigger rare optimization interactions.
