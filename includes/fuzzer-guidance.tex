...
The instrumentation assigns a pseudorandom ID to each basic block and computes edge IDs at runtime by combining the ID of the previous block with the ID of the current block.\fbetodo{Show AFL hash functions}
...
Previous work has demonstrated that the size of the coverage map significantly impacts fuzzing throughput \fbetodo{cite collafl and survey from 2019}.
However, the exact relationship between fuzzing throughput, map size, hardware cache specifics, and counter precision remains a subject of further research \fbetodo{cite vasil AFL paper}.
...
In \XXX we explore an alternative way to incorporate caller context by projecting the context onto code coverage with a compiler transformation.
...
In \XXX we introduce a new form of coverage feedback for compiler fuzzing that incurs less overhead than code coverage, while allowing for more context sensitivity.
