...
Among the most popular forms of feedback is code-coverage.
...
It was \propername{AFL}, however, which popularized the approach for greybox fuzzing by allowing code-coverage to guide the seed selection and mutation process without the overhead of program analysis~\cite{Zalewski2016}.
The tremendous success of \propername{AFL} in bug finding also seemed to confirm this strategy.

The particular flavor of code coverage used by \propername{AFL} and many of its descendants is \emph{edge coverage}.
...
Edge coverage on the other hand records the specific transitions: tuples $(A, B)$ and $(B, C)$ for the first path, versus $(A, C)$ and $(C, B)$ for the second.
...
Despite its widespread use and success, edge coverage, and more general code coverage, suffers from conceptual issues and from implementation costs.
...
On the implementation side, early inefficiencies present in \eg \propername{AFL} were largely solved or at least mitigated with increasingly refined code coverage implementations.
For instance, the initial coverage implementation in \propername{AFL} modified the assembler to instrument targets.
...
A critical edge is an edge from a basic block \emph{with more than one successor} to a basic block with \emph{more than one predecessor}.
For example, consider the critical edge on the left side of \cref{lool:fig:critical-edges}.
The traditional \propername{AFL} instrumentation would have to identify this edge by combining the edge IDs of BB\textsubscript{0} and BB\textsubscript{3}.
...
The refinement of coverage implementations through tighter compiler integration exemplifies the compiler's role in enhancing fuzzing.
...
While there are no published, detailed analyses of the overhead incurred by different compiler instrumentation techniques, \citeauthor{Nagy2019} report an overhead ranging between 30\% and 70\%.
...
We will return to the issue of code-coverage overhead in \cref{ch:lool}.
...
The standard coverage map size in \propername{AFL}, for example, is 64KB, with each byte serving as a counter from 0 to 255.
...
A natural mitigation for this problem is to track paths containing more than one edge, sometimes called \emph{n-gram}~\cite{Wang2019}.
With growing $n$, n-grams approximate full program traces.
The downside of longer n-grams is the performance impact for recording them and a potential queue explosion in the fuzzer.

Another refinement of code coverage is to include caller contexts in the coverage information~\cite{Chen2018a,Wang2019}.
As edge-coverage instrumentation happens only within a function, information about \emph{who} called the function is lost.
...
Angora, for example, addresses this issue by augmenting edge-coverage with a call context hash~\cite{Chen2018a}.
