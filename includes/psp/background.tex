...
\begin{listing}
    \centering
    \begin{minted}[mathescape,escapeinside=||,linenos]{Java}
...
for (int i=0; i<1000; i++) {
    ...
    animal.makeSound(); |\mintedlinelabel{psp:line:polymorphic-call}|
}
    \end{minted}
    \caption{Java program with a polymorphic method call.}
    \label{psp:lst:polymorphic-call}
\end{listing}
\begin{listing}
    \centering
    \begin{minted}{gas}
...
ret
    \end{minted}
    \caption{Conceptual assembly of the call in \cref{psp:lst:polymorphic-call}.}
    \label{psp:lst:vcall-pseudocode}
\end{listing}

While dynamic binding is a powerful feature, the indirect calls required for vtable lookups can introduce performance overhead and hinder other compiler optimizations.
To mitigate this overhead, in particular \glspl{JITcomp} use techniques to devirtualize these calls when possible.
...
For example, consider the Java Program in \cref{psp:lst:polymorphic-call}.
The polymorphic call site in \cref{psp:line:polymorphic-call} calls either \code{Cat}'s or \code{Dog}'s implementation of \code{makeSound}.
On the assembly level, the call site looks like in \cref{psp:lst:vcall-pseudocode}.
...
ret
    \end{minted}
    \caption{Code from \cref{psp:lst:polymorphic-call} with polymorphic inlining.}
