...
The core insight is that compilers already perform sophisticated analyses to reason about data dependencies, be it for optimization or for security hardening.
...
Whether this additional visibility translates into improved fuzzing effectiveness is an empirical question that we investigate in \cref{psp:s:evaluation}.
...
An important observation is that the instrumentation does not reset the current block ID after a call, \eg after the call to \code{handler1} in \cref{psp:line:handler1-call}.
...
All these variants rely on runtime hash computation and are, thus, susceptible to collisions.
