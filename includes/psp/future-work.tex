...
\subsubsection{Backward gradients}
\begin{figure}[t]
    \centering
...
void process_data(uint32_t x) {
  ...
  if (x > 10000) {        // step 1
    ...
    \begin{minipage}[t]{0.48\textwidth}
        ...
        \label{psp:lst:backward-gradients}
    \end{minipage}%
    \hfill
    \begin{minipage}[t]{0.48\textwidth}
...
As discussed in \cref{psp:s:value-range-partitioning}, our \gls{VRP} instrumentation uses existing comparisons as a starting point to insert additional gradients.
...
Consider \cref{psp:lst:backward-gradients} as an example.
Our \gls{VRP} instrumentation would use the \code{x < 12890} comparison as a starting point and insert additional gradients between the two comparisons.
...
Another possibility is to choose a different starting point as well, such as interesting \emph{dataflow sinks}, and to move backwards from there.
...
Note that the backward and forward insertion heuristics are not mutually exclusive, and which one to apply could be based on the size of the value range to partition.
...
\begin{figure}[t]
    ...
    \begin{minipage}[t]{0.48\textwidth}
        \begin{minted}[fontsize=\footnotesize]{C}
void process_data(uint32_t A, uint32_t B) {
  if (A <= B) {
    crash_program();
  }
}
