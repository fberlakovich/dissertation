\section{Implementation}\label{psp:s:implementation}
In this section we describe the relevant implementation details of \glsfirst{PSP}.
Our implementation is based on \propername{LLVM} 16 and consists of about 2500 lines of code in total.
The following subsections describe the implementation of indirect call unfolding and value range partitioning in more detail.
...
In general, static points-to analysis faces theoretical limits (one of the achilles heels of \gls{CFI}), but additional context can help to improve the analysis precision~\cite{Andersen1994,SharirPnueli1981,WilsonLam1995,Milanova2005}.
...
void test() {
    ...
    f1(10);    // Only handle_positive is possible $\label{psp:line:mlta-call-1}$

    // Load and invoke the handler through WrapperB
    handler_t f2 = B.context.exec;
    f2(-3);    // Only handle_negative is possible $\label{psp:line:mlta-call-2}$
}
...
void process_buffer(int idx) {
  ...
  if (idx >= 0 && idx < 100) {
    ...
    \label{psp:lst:offset-tracking-example}
\end{listing}
...
We initially tried a few analyses from the SVF framework but were unsatisfied with the results~\cite{sui2016}.
The purpose of such analyses is to restrict the possible targets at indirect call sites, the so called \emph{target set}.
...
As an example consider the program in \cref{psp:lst:mlta-example}.
A naive points-to analysis based only on the function type would conclude that both handlers, \code{handle\_positive} and \code{handle\_negative} are possible at both call sites in \cref{psp:line:mlta-call-1} and \cref{psp:line:mlta-call-2}.
...
When the program retrieves the function pointer before \cref{psp:line:mlta-call-1}, \gls{MLTA} combines the information about containing structs with the function type information and concludes that the pointer can only lead to \code{handle\_positive}.
...
Our implementation consists of a LLVM pass that implements the splitting point and transformation heuristics described in \cref{psp:s:value-range-partitioning}.
...
void process_buffer(int idx) {
  ...
  if (idx >= 0 && idx < 100) {
    ...
    \label{psp:lst:offset-tracking-example-aliasing}
\end{listing}
...
void process_data(uint32_t x) {
    ...
    \label{psp:lst:constraint-value-range}
\end{listing}
...
void process_buffer(int idx) {
  ...
  if (idx >= 0 && idx < 100) {
    ...
    \begin{minipage}[t]{0.48\textwidth}
        ...
        \label{psp:lst:offset-tracking-example-transformed-naive}
    \end{minipage}%
    \hfill
    \begin{minipage}[t]{0.48\textwidth}
        \subcaption{With offset tracking: split at 90.}
        \label{psp:lst:offset-tracking-example-transformed}
    \end{minipage}
    \caption{Comparison of naive vs.\ offset-aware transformation for \cref{psp:lst:offset-tracking-example}.}
    \label{psp:fig:offset-tracking-comparison}
\end{figure}
...
As an example, consider the code in \cref{psp:lst:constraint-value-range}.
...
Consider, for example, the code in \cref{psp:lst:offset-tracking-example}.
...
The naive implementation would, thus, transform the example to the code shown in \cref{psp:lst:offset-tracking-example-transformed-naive}.
...
In the example in \cref{psp:lst:offset-tracking-example}, the \propername{boundaries} heuristic detects the additional offset of $10$ and instead transforms the program as shown in \cref{psp:lst:offset-tracking-example-transformed}.
\Cref{psp:fig:offset-tracking-comparison} compares both approaches.
Similarly, the heuristics deal with simple forms of casts and aliasing.
If \cref{psp:lst:offset-tracking-example} was written as \cref{psp:lst:offset-tracking-example-aliasing}, for example, the heuristics would still perform the transformation, since the value of \code{idx} flows to the usage.
