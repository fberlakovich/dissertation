...
Here we present an alternative solution to the problem, which we call \emph{Program State Projection} or \glsshortkey{PSP} for short.
...
int main(int argc, char *argv[]) {
  ...
  for (auto shape : parsedShapes) shape->draw(0); |\label{psp:line:broken-draw-call}|
}
    \end{minted}
...
For example, consider the C condition in \cref{psp:lst:c-value-range}.
\begin{listing}
    \begin{minted}{C}
...
For example, compilers use such analyses to eliminate redundant conditions, as seen in line~\ref{psp:line:redundant-condition} of \cref{psp:lst:redundant-condition}.
\begin{listing}
...
void target(int16_t param) {
  if (param < 0) {
    ...
    // redundant condition
    if (param == 0) { |\label{psp:line:redundant-condition}|
    ...
    \label{psp:lst:redundant-condition}
\end{listing}
Assuming that \code{param} is not, \eg \code{volatile}, the compiler can prove that at the time control flow reaches the condition at line~\ref{psp:line:redundant-condition}, \code{param} can never be $0$.
...
For example, a simple compiler transformation can convert \cref{psp:lst:c-value-range} into \cref{psp:lst:c-value-range-transformed}.
...
void target(int16_t param) {
  if (param < 0) {
    ...
    \end{minted}
    \caption{Transformed version of \cref{psp:lst:c-value-range} with artificial branches subdividing each value range.}
    \label{psp:lst:c-value-range-transformed}
\end{listing}
...
In the example in \cref{psp:lst:c-value-range-transformed} $-16384$ and $16384$ are split points.
...
For example, the heuristic transforms code patterns like \cref{psp:lst:c-value-range}, but would not transform patterns like \cref{psp:lst:psc-no-transform} due to the lack of existing comparisons.
This choice is a tradeoff, as functions such as the one in \cref{psp:lst:psc-no-transform} with a flat control-flow and purely value-based bugs are possible and would be missed by the heuristic.
...
void target(int16_t param) {
  ...
  deal_with_value(param);
    ...
    \label{psp:lst:psc-no-transform}
\end{listing}
...
In the example in \cref{psp:lst:psc-no-transform}, we could reason that because the param value flows into a function call, it is relevant to the program.
