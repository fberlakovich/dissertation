\section{Program State Projection}\label{psp:s:program-state-projection}
In \cref{psp:sec:motivation}, we discussed how \aflpp relies on edge coverage instrumentation to guide the fuzzing process.
...
Here we present an alternative solution to the problem, which we call \gls{PSP}.
The core idea of \gls{PSP} is to project parts of the program's hidden data state onto the explicit control-flow graph.
...
This allows the fuzzer to distinguish between states that were previously conflated, potentially providing stepping stones to explore deeper into the state space.
...
Our first application of \gls{PSP} addresses the information loss occurring at indirect call sites.
As discussed in \cref{psp:sec:motivation}, \aflpp's compiler instrumentation for code coverage loses information at indirect call sites because it cannot differentiate the dynamic edges of an indirect call.
...
\glspl{JITcomp} perform a similar optimization called \emph{polymorphic inlining} (see \cref{psp:ss:polymorphic-inlining}), typically based on profiling information.
...
We call this principle \emph{Program State Projection}, or \glsshortkey{PSP} for short, because it combines control flow with parts of the program's hidden state.


\begin{listing}
    \begin{minted}[escapeinside=||]{cpp}
struct Shape { virtual void draw(int scale) = 0; };
struct ShapeParser { virtual Shape *parse(std::string pattern) = 0; };
...
struct CircleParser : ShapeParser {
  Shape *parse(std::string p) override {
    return p == "circle" ? new Circle() : nullptr;
  }
};
struct RectangleParser : ShapeParser {
  Shape *parse(std::string p) override {
    return p == "rectangle" ? new Rectangle() : nullptr;
  }
};

int main(int argc, char *argv[]) {
  std::vector<ShapeParser *> parsers = {new RectangleParser(), new CircleParser()};
  std::vector<Shape *> shapes;
  std::ifstream stream(argv[1]);
  for (std::string line; std::getline(stream, line);)
    for (auto p : parsers) if (Shape *s = p->parse(line)) shapes.push_back(s);

  for (auto shape : shapes) shape->draw(5);
  // filter shapes somehow
  for (auto shape : shapes) shape->draw(0); |\label{psp:line:broken-draw-call}|
}
    \end{minted}
    \caption{Polymorphic call site where a bug in \code{Circle::draw} is masked by prior coverage of both implementations.}
    \label{psp:lst:vcall-dependence}
\end{listing}


\cref{psp:lst:vcall-dependence} shows a simple polymorphic call example in \cpp.
...
Whether the bug is triggered depends on the filtering logic and whether circle objects reach the call site at line~\ref{psp:line:broken-draw-call}.
...
In contrast, \gls{PSP} would conceptually transform line~\ref{psp:line:broken-draw-call} as shown in \cref{psp:lst:broken-draw-call-unfolded}.
\begin{listing}[t]
    \begin{minted}{cpp}
if (dynamic_cast<Rectangle *>(*iter)) {
  // call Rectangle::draw without dynamic dispatch
} else if (dynamic_cast<Circle *>(*iter)) {
  ...
  (*iter)->draw(5);
    ...
    \label{psp:lst:broken-draw-call-unfolded}
\end{listing}
Each added control-flow branch provides the fuzzer, in principle, with a potential stepping stone to build a new seed.


\section{Value-Range Partitioning}\label{psp:s:value-range-partitioning}
\begin{figure}[t]
    \centering
...
For example, consider the C condition in \cref{psp:fig:vrp-original}.
\begin{figure}[t]
    \centering
    % Original code + unpartitioned visualization
...
void target(int16_t param) {
  if (param < 0) {
    process_negative(param);
  } else {
    process_positive(param);
  }
}
            \end{minted}
        \end{minipage}%
        ...
        \begin{minipage}[t]{0.4\textwidth}
            \centering
            \includegraphics[width=\textwidth]{figures/psp/unpartitioned.pdf}
        \end{minipage}
        \caption{Original code. The branch partitions the value range into two edges.}
        \label{psp:fig:vrp-original}
    \end{subfigure}

    \vspace{1em}

    % Transformed code + partitioned visualization
    \begin{subfigure}[t]{\textwidth}
        \centering
        \begin{minipage}[t]{0.48\textwidth}
            \begin{minted}[fontsize=\footnotesize]{C}
void target(int16_t param) {
  if (param < 0) {
    ...
    if (param < 16384) {
        ...
        \begin{minipage}[t]{0.4\textwidth}
            \centering
            \includegraphics[width=\textwidth]{figures/psp/partitioned.pdf}
        \end{minipage}
        \caption{Transformed code with artificial branches subdividing each of the original ranges into two sub-ranges, resulting in four edges.}
        \label{psp:fig:vrp-transformed}
    \end{subfigure}

    \caption{\acrlong{VRP} applied to a simple branch.}
    \label{psp:fig:vrp-transformation}
\end{figure}
...
Value range analysis, for instance, allows compilers to eliminate redundant conditions, \ie nested conditions that are logically implied by the surrounding condition.
We can repurpose this compiler knowledge to map these hidden data-space boundaries onto the control flow, again making them available for edge-coverage instrumentation.
For example, the \gls{VRP} transformation shown in \cref{psp:fig:vrp-transformed} converts the original code into a version with artificial branches.
...
The additional edges provide the fuzzer with an approximation of how well the data space of the variable has been samled.
...
In the example in \cref{psp:fig:vrp-transformed} $-16384$ and $16384$ are split points.
...
\begin{description}
    ...
    This heuristic is suitable if nothing is known about the value range in question or the variable usage.
    ...
    \Cref{psp:fig:boundaries-example} shows an example where the \propername{boundaries} heuristic can help.
    ...
    \Cref{psp:fig:arithmetic-example} shows an example of the \propername{arithmetic} heuristic.
\end{description}
...
void process_request(size_t len) {
  ...
  if (len > 0) {
    ...
    \begin{minipage}[t]{0.48\textwidth}
        \subcaption{Original code.}
        \label{psp:lst:boundaries-example}
    \end{minipage}%
    \hfill
    \begin{minipage}[t]{0.48\textwidth}
        \subcaption{After transformation.}
        \label{psp:lst:boundaries-example-transformed}
    \end{minipage}
    \caption{The \propername{boundaries} heuristic splits at the buffer size boundary (100).}
    \label{psp:fig:boundaries-example}
\end{figure}
...
Since we must duplicate the code in both branches (cf. \cref{psp:fig:arithmetic-example}), however, larger branches mean more code bloat.
...
For example, the heuristic transforms code patterns like \cref{psp:fig:vrp-original}, but would not transform patterns like \cref{psp:lst:psp-no-transform} due to the lack of existing comparisons.
This choice is a tradeoff, as functions such as the one in \cref{psp:lst:psp-no-transform} with a flat control-flow and purely value-based bugs are possible and would be missed by the heuristic.
We discuss an alternative insertion heuristic in \cref{psp:sss:insertion-points}.

Beyond implementation simplicity, choosing existing comparisons as insertion points has further advantages.
...
void target(int16_t param) {
  ...
  process_value(param);
    ...
    \label{psp:lst:psp-no-transform}
\end{listing}
