...
Two performance counters are particularly important when interpreting the results: cycles, instructions and Intel's \gls{TMA} counters\cite{yasin2014}.
...
At the same time, superscalar pipelining \glspl{CPU} (all of the \glspl{CPU} in our evaluation), overlap instruction execution and can execute multiple micro-operations in parallel, depending on the instruction type.
...
\propername{Gcc} shows a combination of the phenomenons discussed so far.
...
\Cref{r2c:tab:call-density} shows that \propername{x264} has the lowest call density (2.7 calls per 1000 instructions) due to its loop-intensive video encoding kernels, followed by \propername{deepsjeng} and \propername{leela} (7.0 and 7.2 respectively) which execute about 140 instructions between calls on average.
...
The high \propername{Bad Speculation} rate correlates with low baseline utilization but does not itself create slack; it explains why the baseline \gls{IPC} is low, not why the overhead is hidden.
...
\begin{table}[t]
    ...
    \begin{tabular}{l
        ...
        \midrule
        \propername{x264}      & 866.8                & 2.3         & 2.7       \\
        \propername{deepsjeng} & 1635.1               & 11.4        & 7.0       \\
        \propername{leela}     & 1638.0               & 11.8        & 7.2       \\
