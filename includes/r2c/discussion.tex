...
The data suggests that there is only a weak correlation with the overhead:
...
To better understand the exact causes of the overhead, we recorded \code{perf} counters for particularly affected benchmarks.
...
\end{enumerate*}
While it is generally hard to differentiate the two effects, we can correlate the instruction cache pressure increase with the performance overhead increase.
