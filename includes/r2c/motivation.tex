\section{Motivation}\label{r2c:s:aocr}
...
While indirect JIT-ROP focuses on the discovery of gadgets, address-oblivious code-reuse changes the reuse granularity back from sub-function-level reuse to whole-function reuse.
...
\begin{inline}
    ...
    \item the function's control-flow must be controllable via memory corruptions, \eg by corrupting the function's parameters.
    In their attack examples, the \gls{AOCR} authors exploit functions that read parameter values from memory.
    Examples of such functions are default parameter wrappers, which take fewer parameters than the original function and pass a default value read from memory to the original function.
\end{inline}
...
The stack contains many code pointers (return addresses and function pointers) as well as pointers to the heap and thus serves as a stepping stone to reach other data areas.
...
Based on these observations, we gain two important insights from the \gls{AOCR} paper:
\begin{inline}
    \item \gls{CPH}, while providing security guarantees similar to coarse-grained \gls{CFI}, introduces new problems with its trampoline table;
    \item code randomization without data-layout randomization is insufficient to prevent inference attacks.
\end{inline}

These insights motivate our compiler-based defense \rtwoc: a defense that extends the principle of randomization from code to observable data.
The compiler possesses precise knowledge of stack frame geometry, including the positions of return addresses, spilled registers, and local variables.
...
In the following sections we explore the concept of mimicry in detail and provide a detailed evaluation of \rtwoc's performance overhead and security guarantees.

\begin{figure}[t]
    \centering
    \includeDrawioFigure{figures/r2c/overview}{
        \draw ([xshift=2pt] PointA) node {\circledtikz{\code{\footnotesize A}}};
        \draw ([xshift=2pt] PointB) node {\circledtikz{\code{\footnotesize B}}};
    ...
    \caption{Prior systems primarily diversify code, leaving the layout of observable data predictable (left vs middle, see \cref{r2c:s:aocr}).
