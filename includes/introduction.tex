...
Modern web browser, for example, consist of 20 or even 30 million lines of code~\cite{openhub-firefox,openhub-chromium}.
...
Already in the year 2025, \gls{LLM}s can reduce the cost of creating highly sophisticated code-reuse exploits from days or even weeks down to a few hours and an investment of a few euros, and this is likely only the beginning.
...
\begin{figure}[t]
    \centering
    \input{figures/coevolution/code-reuse-trend.tex}
...
Finally, I present \emph{Program State Projection} (PSP), a technique that uses compiler analyses to expose hidden data dependencies to coverage-guided fuzzers.
...
Our evaluation investigates whether these additional stepping stones translate into measurable improvements on real-world benchmarks.


We close each part with an outlook, providing more general insights and possible future research directions.