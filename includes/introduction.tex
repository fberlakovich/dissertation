...
Nonetheless, despite the rise of languages like , C and \cpp remain dominant, consistently ranking in the top tiers of language popularity indices. \todo{Cite TIOBE index}
...
In the first part, I present \rtwoc, a novel, compiler-driven software diversity defense that addresses the challenge of layout inference through information leaks.
It combines fine-grained code randomization with targeted randomization of program data structures.
By diversifying not just the code but also the data layout that recent attacks rely on for reconnaissance, \rtwoc renders such attacks ineffective while incurring a moderate performance overhead.

In the second part, I present \lool, a compiler-assisted fuzzing framework based on a new type of coverage feedback.
...
A genetic algorithm uses this feedback to systematically steer testcase generation towards rare and complex optimization interactions that are difficult to trigger otherwise.
