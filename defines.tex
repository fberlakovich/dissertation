...
% Load external table data for use inside tabularray tables.
% CatchFileDef reads with correct catcodes (& as alignment tab).
% Usage: \loadTableData{file} before the table, then \tabledata inside the body.
\usepackage{catchfile}
\newcommand{\loadTableData}[1]{\CatchFileDef{\tabledata}{#1}{}}
...
\newcommand{\propernamedecl}{\sffamily\hyphenchar\font=`-\relax}
\newcommand{\propername}[1]{{\propernamedecl#1}}
\newcommand{\afl}{\propername{AFL}\xspace}
\newcommand{\aflpp}{\propername{AFL}\plusplus}


% @formatter:off
\definecolor{tablerowgray}{gray}{0.96}

% Custom tabularray key: headrows=N declares N header rows.
% Zebra striping starts after the headers; first data row is always white.
\ExplSyntaxOn
\DeclareTblrKeys { table/inner } {
    headrows .code:n = {
        \int_if_odd:nTF { #1 }
        {
            \keys_set:nn { tabularray/table/inner } { row{odd} = {bg=tablerowgray} }
            \int_step_inline:nnnn { 1 } { 2 } { #1 }
            { \keys_set:nn { tabularray/table/inner } { row{##1} = {bg=white} } }
        }
        {
            \keys_set:nn { tabularray/table/inner } { row{even} = {bg=tablerowgray} }
...
% Shaded cell command - uses tabularray's SetCell for bg in thesistable
\newcommand{\shadecell}[2]{\SetCell{bg=#1}#2}
