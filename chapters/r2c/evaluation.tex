...
\begin{figure}[t!]
    ...
    \resizebox{\columnwidth}{!}{%
        \input{figures/r2c-exec-time.pgf}%
    }
    \caption{The performance impact of full protection with \rtwoc on four different machines (see \cref{sss:full-overhead}).}
...
To measure the worst-case overhead, we also enabled \btras for call sites to unprotected code (see \cref{ss:limitations-coverage}).
...
Since the location of return addresses and the distribution of \heapbts is random, we recompiled the benchmarks with a different seed for each of the executions.
...
\subsubsection{\btras}\label{sss:eval-btra}
We evaluated the overhead of \btras with the \code{push} setup and with our optimized AVX2 setup sequence respectively.
To isolate the overhead of \btras, we disabled other diversification measures.
We configured \rtwoc{} to instrument each call site with a total of 10 \btras and between 1 and 9 NOPs (see \cref{ss:strengthening}).

For the \code{push} setup, 10 \btras mean that \rtwoc{} inserts up to 12 push instructions per call site: 10 for the \btras, one for the return address, and one to keep the stack aligned (see \cref{ss:impl-rads}).
...
In contrast to the \code{push} instructions, setting up 10 \btras with AVX2 instructions requires only 7 instructions (see \cref{ss:impl-rads}).
...
These numbers suggest that the majority of the overhead is caused by writing the \btras to the stack.

%\subsubsection{\btras with vector instructions}
...
%In contrast to the \code{push} instructions, setting up ten \btras with AVX2 instructions requires only 7 instructions (see \cref{ss:impl-rads}).
...
\subsubsection{\heapbts}\label{sss:eval-heapbt}
We configured \rtwoc{} to insert between zero and five \heapbts per function, but disabled other diversification measures.
\fbetodo{adjust to new numbers}
\cref{tab:perf-components} shows that the geometric mean overhead of \heapbts is 2\% with \propername{xalancbmk} causing the maximum overhead of 5\%.
The optimization to insert \heapbts only in functions that write to their stack frame improves performance by 1\%.
...
%\item protects return addresses with \btras (see \cref{ss:impl-rads});
%\item injects \heapbts (see \cref{ss:impl-heap-boobytraps});
...
We verified experimentally that about 55\% of the memory overhead is caused by the page allocations for \heapbts.
The rest is caused by \btras and the increased binary size.
...
We built both browsers with a fixed total number of 10 \btras per call site.
