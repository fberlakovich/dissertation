...
\begin{enumerate*}[label=(\roman*)]
    ...
    \item instruments functions to insert \glspl{heapbt}, which thwart the statistical analysis of pointers on the stack (see \cref{ss:impl-heap-boobytraps}).
\end{enumerate*}

\subsection{BTRAs: Booby-Trapped Return Addresses}\label{ss:impl-rads}
% To this end, \rtwoc encloses return address with BTRAs.
To enclose return addresses with \glspl{BTRA}, steps \Circled{\code{1}} to \Circled{\code{4}} in \cref{fig:rad-insertion} are required.
...
To improve \rtwoc{}'s performance, we built an optimized variant that sets up the \glspl{BTRA} and the return address with AVX2 vector instructions (see \cref{fig:avx2-setup})~\cite{IntelAVX}.
...
\includegraphics{figures/r2c/hardened-btdps}
}
\begin{tikzpicture}[overlay, remember picture]
...
The \glspl{heapbt} \propername{X} and \propername{Y} mislead attempts to disclose the \gls{heapbt} array pointer from the data section (see \cref{ss:impl-heap-boobytraps}).}
...
\subsection{\glspl{heapbt}: Booby-Trap Data Pointers}\label{ss:impl-heap-boobytraps}
...
We therefore simulate the desired effect by letting \glspl{heapbt} point to random offsets in \emph{guard pages} allocated from heap memory.
...
During compilation, \rtwoc{} instruments functions to write \glspl{heapbt} from the pointer array to the stack.
How many \glspl{heapbt} are written per function is chosen randomly chosen using compile-time parameters.
The stack slots for the \glspl{heapbt} are allocated like stack slots for local variables.
...
%    Thus, when storing the \glspl{heapbt} directly in the data section, \rtwoc requires two instructions to write a single \heapbt to the stack.
%    If the attacker has access to the data section (\ie knows its location), storing the \glspl{heapbt} in the data section might pose a security risk.
...
%    Pointers observed in both locations could potentially be \glspl{heapbt}.
...
To protect \glspl{heapbt} against such an attack scenario, \rtwoc{} applies \rtwoc{}'s principle also to the array holding the \glspl{heapbt}.
...
The attacker thus loses the ability to reliably identify \glspl{heapbt}.
