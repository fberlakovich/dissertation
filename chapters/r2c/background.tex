...
Before \cras, the most straightforward way for an attacker to execute arbitrary code was to use a memory corruption to inject code into the target process.
...
With the widespread adoption of \gls{DEP}, attackers switched to \cras.

Unlike code injection, a \cra reuses the code that is already present in the target process.
Initially \cras operated on the function level and \eg reused functions from the standard C library (a technique called \propername{return-to-libc}).
