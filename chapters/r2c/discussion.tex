...
As evidenced by the benchmarks, the optimized \gls{btra} setup sequence improves \prj{}'s performance considerably (see \cref{tab:perf-components}).
...
\Cref{fig:perf-full} shows that benchmarks with a large number of functions and function calls are affected most by \prj{}.
\prj{} adds \glspl{btra} \emph{per call site}, explaining the overhead for function heavy benchmarks.
...
We suspect that for the SPEC benchmarks, memory overhead caused by \prj{} is low compared to the memory consumed by the benchmark itself.
...
In the following subsections we discuss how \prj{} counters each of these attack vectors.
...
\prj protects return addresses with \glspl{btra}.
...
\prj{}'s additional code randomization (see \cref{ss:strengthening}) means that an attacker cannot reliably infer the address of the \emph{calling} function based on a leaked return address.
...
\prj protects function pointers on the stack and in the data section with stack slot shuffling and global variable shuffling, respectively.
\prj{}'s additional code randomization (see \cref{ss:strengthening}) ensures that such pointers cannot be used to locate gadgets, effectively forcing the attacker into a more constrained whole-function reuse setting.
...
Although \prj{} does not protect against leaks from a heap over-read, it does, however, impede the disclosure of heap pointers with \glspl{heapbt}.
...
$B$ on the other hand is a random variable with a uniform distribution that depends on \prj{}'s parameters (see \cref{ss:impl-heap-boobytraps}).
...
As part of the \gls{btra} setup and teardown code, \prj{} also emits the necessary \code{CFI} directives to support exception handling and stack unwinding.
...
\prj{} impedes a \gls{PIROP} attack in two ways.
First, \prj{} shuffles functions and randomizes at the sub-function level (see \cref{ss:strengthening}), thus increasing the entropy for \gls{PIROP}.
...
At present, \prj{} remains susceptible to two types of brute force attacks.
In a Blind ROP scenario with restarting worker processes, an attacker could use \gls{PIROP} to brute force the entropy resulting from \prj{} randomization techniques.
...
\prj{} could also deter the corruption of \glspl{btra} by checking a random subset of \glspl{btra} for consistency after the return.

\prj{} focuses on code-reuse attacks and the leakage of \emph{control-flow} data.
Although \prj{}'s layout randomization raises the bar for attackers~\cite{Hu2016}, \prj{} does not offer the same protection as defenses specialized for data-only attacks~\cite{Carr}.

A way to strengthen \prj{}'s security would be to combine it with \gls{MVEE}s~\cite{Cox2006,Berger2006,Bruschi2007,Volckaert2016,Voulimeneas2020}.
\gls{MVEE}s and diversification defenses like \prj{} naturally complement each other.
Considering that \prj{} diversifies along multiple dimensions, an \gls{MVEE} would detect data corruption or leakage in one of the variants with high probability.

\subsection{Limitations}\label{ss:limitations}

\subsubsection{Coverage}\label{ss:limitations-coverage}
\prj{} is able to protect only the call-sites and functions it actually compiles.
...
Overwritten \glspl{btra} are only an issue, however, if the attacker knows which parts of the program have not been compiled by \prj.
Lacking this information, an attacker does not know where the return address is the last address.

\subsubsection{Support for stack argument calling convention with non-\prj{} compiled code}\label{ss:abi-change}
At present, our prototype implementation does not support calling functions with stack arguments from code not compiled by \prj.
...
During our evaluation of \prj{}, we encountered just three such cases (one in the unit tests of WebKit, one in the XML parser callbacks of WebKit, and one in the regular expression implementation of Chromium).
