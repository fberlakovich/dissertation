\section{Design and Implementation of \lool[noformat]{}}\label{s:lool-design}

In \cref{sec:graalvm_fuzzing} we described how our GraalVM fuzzer solves three of the five challenges described in \cref{ss:jit-compiler-fuzzing} (\cref{problem_correct_inputs,problem_compiled_inputs,problem_miscompilations}).
For the remaining challenges (\cref{problem_new_coverage,problem_steer_input}), we propose a new, domain-specific approach.
\lool{} uses the compiler's optimization log (see \cref{sss:graalvm-optimization-log}) to select among suitable parameter vectors.
The selection process is driven by a genetic algorithm that tries to breed more desirable parameter vectors over time.
\cref{fig:lool-overview} shows an overview of the interaction between the genetic algorithm, the input generator, and the optimization log.
...
\begin{figure}
    ...
    The differently shaded areas differentiate the GraalVM fuzzing framework (see \cref{sec:graalvm_fuzzing}) from \lool's components (see \cref{s:lool-design}).}
    \label{fig:lool-overview}
\end{figure}

\subsection{Overview}\label{ss:design-overview}
...
We will compare both approaches to input mutation as part of our evaluation (see \cref{s:evaluation}).

\begin{figure}[t]
    \centering
    \includegraphics[width=0.6\linewidth]{figures/lool/lool-workflow}
    \caption{Workflow of the genetic algorithm loop (see \cref{ss:fitness} and \cref{ss:building-generations}).}\label{fig:workflow}
\end{figure}
...
\subsection{Fitness}\label{ss:fitness}
...
\subsubsection{Optimization Counter Fitness}\label{sss:opt-counter-fitness}
...
\subsection{Building Generations}\label{ss:building-generations}
As shown in \cref{fig:workflow}, the genetic algorithm mutates and crossbreeds parameter vectors to form a new generation.
...
As we only include a subset of the distribution in the parameter vectors (see \cref{sec:search_space}), we equally distribute the difference to 1 among the excluded probabilities.
...
\cref{fig:gen_representation} shows an abbreviated example of a parameter vector before and after mutation.
...
\begin{table}{}
    ...
    % FIXME: show the full table (maybe in the final version) to make the numbers less confusing?
    \label{fig:gen_representation}
...
\subsection{\aflpp-compatible coverage}
In our evaluation (see \cref{s:evaluation}) we evaluate various configurations that use \aflpp as the mutation engine.
\aflpp expects a byte buffer as coverage in which each byte represents a unique counter from 0 to 255.
In a standard \aflpp setup, this coverage map is provided by a binary instrumented for code coverage and each byte represents the execution count of a specific edge id.
...
In the case of context-aware coverage (per-method optimization counters), only the number of optimization counters per method is known a priori.
The total number of optimization counters depends on the number of compiled methods and varies from test to test.
We calculate an index based on the compiled method's name and then assign each optimization counter a unique position based on its name starting from the method's index.
For example, consider two tests with methods \propername{A}, \propername{B}, \propername{C} and \propername{A}, \propername{B}, \propername{D}.
...
Depending on the number of compiled methods, this approach likely leads to coverage collisions, as each method receives a fixed block of counter indices.
We note, however, that this problem is not specific to using optimization counter coverage, but also occurs \eg when using \aflpp code coverage instrumentation without \gls{LTO}.

\subsection{Parameter Mutation by \aflpp}\label{ss:afl-parameter-mutation}
In our evaluation (see \cref{s:evaluation}), various configurations use \aflpp as the mutation engine.
...
We have implemented and evaluated two ways in the program generator to derive generation parameters and random decisions from \aflpp generated input files.

\subsubsection{Fully Parametric}\label{sss:afl-fully-parametric}
The program generator reads both the generation parameters and concrete random decisions directly from the input file.
If the generator runs out of randomness, \ie the input is too short to generate a valid live program, the generator signals an error and tries again.
On each retry, the generator reduces structural parameters determining the size of the generated program (\eg the number of statements in a method) by 50\%.
If all structural parameters have already reached their limit, the generator signals an error and continues with the next input.
How often such a reduction is possible, depends on the initial parameters.
We implemented this approach because with certain program features the generator needs to perform backtracking to find valid programs with a limited set of live variables.
Although we optimized this backtracking to use only 4 bytes of randomness in case of a failed attempt, multiple failed attempts can use up the available randomness quickly.
We discuss the problems with this approach in \cref{ss:aflpp-vs-genetic}.

\subsubsection{Generation Parameters Only}\label{sss:afl-hyperonly}
The program generator reads only the generation parameters and a single integer directly from the file.
The integer serves as random seed for a new, in-memory random instance that feeds the concrete random decisions.
This approach allows \aflpp to mutate the generation parameters in a more targeted way.
Reading the seed from the input file is important to guarantee the same generated program (and thus the same coverage feedback) given the same input file.
